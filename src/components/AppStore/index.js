...
class AppStore extends Component {
  ...
  changeTab = (tabId) => {
    const { tabActive, searchInput } = this.state
    this.setState({
      tabActive: tabId,
    })
  ...
  render() {
    ...
    const filterApps = appsList.filter(eachItem => (eachItem.category === tabActive))


    const searchFilter = filterApps.filter(eachItem => eachItem.appName.toLowerCase().includes(searchInput.toLowerCase()))


    return (
      <div className="bg-gradient-to-b from-gray-300 via-red-300 to-pink-500 h-full flex flex-row justify-center items-center h-screen text-center p-10">
        <div className="h-full">
          <h1 className="text-3xl font-bold text-gray-900">App Store</h1>
          <input onChange={this.searchStore} type="search" className="py-2 m-3 px-6 border-2 border-gray-100 placeholder-gray-100 rounded bg-transparent" placeholder="search" />

          <div className="flex flex-row justify-center">
            {tabsList.map(eachItem => (
              <TabItem isActive={eachItem.tabId === tabActive} changeTab={this.changeTab} key={eachItem.tabId} tabsList={eachItem} />
            ))}
          </div>

          <div className="flex flex-wrap justify-center items-center pt-4 ">
            {searchFilter.map(eachItem => (
              <AppItem tabActive={tabActive} key={eachItem.appId} appsList={eachItem} />
            ))}
          </div>
